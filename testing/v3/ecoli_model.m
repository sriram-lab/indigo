...
% for i=1:length(data_files)
test_data = data_files{length(data_files)};
training_data = data_files([1:length(data_files)-1 length(data_files)+1:end]);
indigo_summary = indigo_run(test_data,training_data,data_lookup,valmethod,K,standardize,model_type,input_type,scoring);
[stats,averages,overview] = analyze(indigo_summary,length(data_files), indigo_data, data_files,1);
save(save_indigo(indigo_summary, 1))
[stats,averages,overview] = analyze(indigo_summary,length(data_files), indigo_data, data_files,2);
save(save_indigo(indigo_summary, 2))
% end
